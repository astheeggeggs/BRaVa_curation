...
do
	for pop in "${pops[@]}"
	do
	    echo "sbatch --export=pheno=$phenotype,pop=$pop,sex=both_sexes qq_and_manhattan_template.sh"
...
do
	for pop in "${pops[@]}"
	do
		sbatch --export=pheno=$phenotype,pop=$pop,sex="XX" qq_and_manhattan_template.sh
	done
done
