...
{
	files <- gsub("@", "[0-9]*X?", file_grep)
	bin <- bin_type
	dt_list <- list()
	for (file in grep(files, dir(count_directory, full.names=TRUE),value=TRUE)) {
...
levels_list <- list(
	...
	MAC=c("singletons", "(1,5]"å,  "(5,10]", "(10,100]" , "(100,1,000]", "(1,000,10,000]", ">10,000"),
	spliceAI=c("<0.2", "[0.2,0.5)", "[0.5,0.8)", ">0.8")
)

# BRaVa annotations
pdf(file="AC_and_variant_counts.pdf", width=10, height=6)
for (annotation_level in c("variant")) { #c("variant", "transcript")) {
	dt_list <- list()
	for (type in c("MAF", "MAC")) {
		for (pop in c("EAS", "AMR", "AFR", "EUR", "SAS")) {
			dt_list[[type]][[pop]] <- extract_overall_counts(
				paste0("ukb_wes_450k.", pop, ".chr@.BRaVa_annotations_", annotation_level, "_summary.tsv.gz"),
				bin_type=type)
