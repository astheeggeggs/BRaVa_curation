...
# Input files
SAMPLE_QC_FILE <- paste0("/well/lindgren/UKBIOBANK/dpalmer/wes_", TRANCHE, "/ukb_wes_qc/data/samples/10_sample_metrics_for_plotting_chr", CHR, ".tsv.gz")
VARIANT_QC_FILE <- paste0("/well/lindgren/UKBIOBANK/dpalmer/wes_", TRANCHE, "/ukb_wes_qc/data/variants/10_variant_metrics_for_plotting_chr", CHR, ".tsv.gz")

# Output files
COMBINED_SAMPLE_QC_FILE <- paste0('/well/lindgren/UKBIOBANK/dpalmer/wes_', TRANCHE, '/ukb_wes_qc/data/10_sample_metrics_for_plotting.tsv')
COMBINED_VARIANT_QC_FILE <- paste0('/well/lindgren/UKBIOBANK/dpalmer/wes_', TRANCHE, '/ukb_wes_qc/data/10_variant_metrics_for_plotting.tsv')
...
ggsave(paste0(PLOTS, TRANCHE, '_10_pHWE_hist.pdf'), p, width=160, height=90, units='mm')
ggsave(paste0(PLOTS, TRANCHE, '_10_pHWE_hist.jpg'), p, width=160, height=90, units='mm')
...
p <- create_pretty_cumulative(dt, aes(x=variant_qc.p_value_hwe), x_label='p(HWE)', threshold=NULL,
    ...
    # labels = scales::trans_format("log10", scales::math_format(10^.x))
  # )
ggsave(paste0(PLOTS, TRANCHE, '_10_pHWE_cdf.pdf'), p, width=160, height=90, units='mm')
ggsave(paste0(PLOTS, TRANCHE, '_10_pHWE_cdf.jpg'), p, width=160, height=90, units='mm')
...
for (CHR in c(seq(2,22), "X")) {
    ...
    SAMPLE_QC_FILE <- paste0(
        ...
        "/ukb_wes_qc/data/samples/10_sample_metrics_for_plotting_chr", CHR, ".tsv.gz")
