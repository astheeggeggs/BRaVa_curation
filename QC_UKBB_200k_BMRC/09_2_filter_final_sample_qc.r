...
# Inputs
SAMPLE_BEFORE_AFTER_COMBINED_QC_FILE <- '/well/lindgren/UKBIOBANK/dpalmer/wes_' + TRANCHE + '/ukb_wes_qc/data/samples/09_final_qc.before.after.combined.samples.tsv'

# Outputs
FINAL_SAMPLE_SUMMARY <- '/well/lindgren/UKBIOBANK/dpalmer/wes_' + TRANCHE + '/ukb_wes_qc/data/samples/09_final_sample.summary.tsv'
...
# rTiTv
p <- ggplot(dt_after, aes(x=sample_qc.r_ti_tv, color=factor(population))) + geom_density(stat="density") + theme_classic() + labs(color='1000G label', x='Transition/Transversion') 
ggsave(paste0(PLOTS, '09_r_ti_tv_density', '.pdf'), p, width=160, height=90, units='mm')
...
# rHetHomVar
p <- ggplot(dt_after, aes(x=sample_qc.r_het_hom_var, color=factor(population))) + geom_density(stat="density") + theme_classic() + labs(color='1000G label', x='(Het)/(Hom var)') 
ggsave(paste0(PLOTS, '09_r_Het_HomVar_density', '.pdf'), p, width=160, height=90, units='mm')
...
# rInsertionDeletion
p <- ggplot(dt_after, aes(x=sample_qc.r_insertion_deletion, color=factor(population))) + geom_density(stat="density") + theme_classic() + labs(color='1000G label', x='Insertion/Deletion') 
ggsave(paste0(PLOTS, '09_r_insertion_deletion_density', '.pdf'), p, width=160, height=90, units='mm')
...
ggsave(paste0(PLOTS, '09_n_singletons_density', '.pdf'), p, width=160, height=90, units='mm')
